def string_compare(s1, s2):
    ...
    """
    #if statement is O(1), for loop with a regular if, is O(n). Both happen independently, so O(n) overall. 
    if len(s1) != len(s2):
        return False
...
def has_exotic_animals(animals):
    ...
    """
    #This if statement acts as two parallel for loops, so O(n)
...
def sum_zero_1(numbers):
    ...
    """
    #Looking up an element in a set: O(1).
    #For looping a set: O(n).
    #Creating a set: O(n).
    #Overall: O(n)
    result = []
...
def sum_zero_2(numbers):
    ...
    """
    #Two nested for loops traversing the same list.
    result = []
...
def sum_zero_3(numbers):
    ...
    """
    #The if statement inside inner for loop is traversing the list.
    result = []
