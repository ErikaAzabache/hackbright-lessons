...
SELECT models.name AS model_name, brands.name AS brands_name, headquarters
FROM models JOIN brands USING (brand_id) WHERE brands.name='Ford' AND models.name = 'Mustang';
...
SELECT brands.name, founded FROM brands LEFT JOIN models USING (brand_id) WHERE models.brand_id IS NULL;
